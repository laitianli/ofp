               ...
               unsigned char *laddr, int vrf)
{
    ...
    if (if_entry->ifa_family == AF_INET)    {
        if (ofp_if_type(dev) == OFP_IFT_GRE) {
            ...
            ofp_set_route_params(OFP_ROUTE_ADD, dev->vrf, dev->vlan, dev->port,
                    *((uint32_t *)laddr), 32, 0,
                    OFP_RTF_LOCAL);
        } else {
            if(dev->vlan == 0)
        ...
        if (ofp_if_type(dev) == OFP_IFT_VXLAN) {
            ...
            ofp_set_route_params(OFP_ROUTE_ADD, dev->vrf, dev->vlan, VXLAN_PORTS,
                         *((uint32_t *)addr), masklen, 0, OFP_RTF_LOCAL);
        ...
        if (ofp_if_type(dev) != OFP_IFT_GRE) {
            ofp_set_route_params(OFP_ROUTE_ADD, dev->vrf, dev->vlan, dev->port,
                    *((uint32_t *)addr), 32, 0, OFP_RTF_LOCAL);
        }
        dev_local = ofp_get_ifnet(dev->port, dev->vlan);
        if (NULL != dev_local)
            ofp_ifnet_ip_find_update_fields(dev_local, *((uint32_t *)addr), masklen, bcast_addr);
    }
#ifdef INET6
               ...
               unsigned char *addr, unsigned char *laddr)
{
    ...
    if (if_entry->ifa_family == AF_INET)    {
        ...
        OFP_DBG("DEL ADDR addr=%x laddr=%x", *((uint32_t *)addr),
            *((uint32_t *)laddr));
        int idx = ofp_ifnet_ip_find(dev, *((uint32_t *)addr));
        if (-1 == idx)
            ...
            ofp_set_route_params(
                    ...
                    *((uint32_t *)addr),
                    dev->ip_addr_info[idx].masklen, 0 /*gw*/, 0);
            ofp_set_route_params(
                    OFP_ROUTE_DEL, dev->vrf, dev->vlan,dev->port,
                    *((uint32_t *)addr),
                    32, 0 /*gw*/, 0);
