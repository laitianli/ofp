...
int sp_rx_thread(void *ifnet_void)
{
    ...
    while (ofp_global_cfg->is_running) {
        ...
        OFP_DBG("send packet len:%u to kernel protocol stack, ethtype：0x%x\n",
            odp_packet_len(pkt), ntohs(eth->ether_type));
        len = write(ifnet->fd,
...
drop_pkg:
        ...
        if (len > 0) {
            ...
            eth = odp_packet_l2_ptr(pkt, NULL);
            OFP_DBG("send to dpdk, len:%d, ethtype: 0x%x\n", len, ntohs(eth->ether_type));
